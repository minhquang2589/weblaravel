...
<div class="my-5 mb-3 mx-3 flex justify-center	">
  <a href="">
    <div class="font-medium text-sm text-gray-600 hover:text-gray-800">Products</div>
  </a>
</div>
<!--  -->
...
<div class="grid grid-cols-1 lg:grid-cols-6 lg:px-2">
  ...
  <div class="h-fit moving rounded-lg  lg:col-span-5">
    <div id="productViewContainer" class="grid grid-cols-2 lg:mx-6 lg:grid-cols-4 2xl:grid-cols-5 lg:gap-2 "></div>
  </div>
</div>
<div class="flex mt-2 w-full items-center justify-center ">
      <button class="text-gray-800  hover:underline" id="loadMoreProductViewButton">Load more</button>
    </div>
...
<script>
  document.addEventListener("DOMContentLoaded", function() {
    ...
    resetButton.addEventListener('click', function() {
      ...
      getProductView(sale,newProduct, instock, outofstock, priceFrom, priceTo, search);
    });
  });
  ...
  var productViewPage = 1;
  var productViewisLoading = false;

  function getProductView(sale,newProduct, instock, outofstock, priceFrom, priceTo, search) {
    if (productViewisLoading) return;
    productViewisLoading = true;
    $.ajax({
      type: 'GET',
      url: "{{ route('get.productview.product') }}",
      data: {
      ...
      success: function(response) {
        if (response.success) {
          if (response.productViews.length > 0) {
            ...
            $('#productViewContainer').append(productsHtml);
            productViewisLoading = false;
            productViewPage++;
            if (response.hasMore == false) {
              $('#loadMoreProductViewButton').text('End').prop('disabled', true).addClass('sold-out-ticket');
            }
          }
        }
      },
  ...
  $('#loadMoreProductViewButton').click(function() {
    getProductView()
  });
</script>
@endsection