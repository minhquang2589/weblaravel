...
@section('content')
@if(Request::path()=='product/bestsellers')
@include('layout.slider')
@endif
<div class="my-5 mb-3 mx-3 flex justify-center	">
  <a href="">
    <div class="font-medium text-sm text-gray-600 hover:text-gray-800">Best Sellers</div>
  </a>
</div>
<!--  -->
...
<div class="my-2 mx-10 mt-3 ml-6">
  <nav aria-label="Breadcrumb">
    <ol class="flex items-center gap-1 text-sm text-gray-600">
      ...
      <li>
        <a href="" class="block transition hover:text-gray-700"> best sellers </a>
      </li>
    </ol>
  </nav>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-6 lg:px-2">
  ...
  <div class="h-fit moving rounded-lg  lg:col-span-5">
    <div id="BestSellContainer" class="grid grid-cols-2 lg:mx-6 lg:grid-cols-4 2xl:grid-cols-5 lg:gap-2 "></div>
    <div class="flex mt-2 w-full items-center justify-center ">
      <button class="text-gray-800  hover:underline" id="loadMoreBestSellProductButton">Load more</button>
    </div>
  </div>
</div>
<script>
  ...
  ////get bestsell product
  var bestsellPage = 1;
  var bestsellisLoading = false;

  function getBestSellProduct() {
    if (bestsellisLoading) return;
    bestsellisLoading = true;
    $.ajax({
      type: 'GET',
      url: "{{ route('get.bestsell.product') }}",
      data: {
        page: bestsellPage
      },
      success: function(response) {
        if (response.success) {
          if (response.bestsell.length > 0) {
            var products = response.bestsell;
            var productsHtml = '';
            ...
            $('#BestSellContainer').append(productsHtml);
            bestsellisLoading = false;
            bestsellPage++;
          } else {
            $('#loadMoreBestSellProductButton').text('End').prop('disabled', true).addClass('sold-out-ticket');
          }
        }
      },
  ...
  $('#loadMoreBestSellProductButton').click(function() {
    getBestSellProduct();
  });
</script>
