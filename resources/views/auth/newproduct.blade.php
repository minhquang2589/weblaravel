...
@section('content')
@if(Request::path()=='new/product')
@include('layout.slider')
@endif
<div class="my-5 mb-3 mx-3 flex justify-center mt-5	">
   <a href="">
...
<div class="my-2 mx-10 mt-3 ml-6">
   <nav aria-label="Breadcrumb">
      <ol class="flex items-center gap-1 text-sm text-gray-600">
         ...
         <li>
            <a href="" class="block transition hover:text-gray-700">New Product </a>
         </li>
      </ol>
   </nav>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-6 lg:px-2">
   ...
   <div class="h-fit moving rounded-lg  lg:col-span-5">
      <div id="newProductContainer" class="grid grid-cols-2 lg:mx-6 lg:grid-cols-4 2xl:grid-cols-5 lg:gap-8 "></div>
      <div class="flex mt-2 w-full items-center justify-center ">
         <button class="text-gray-800  hover:underline" id="loadMoreNewProductButton">Load more</button>
      </div>
   </div>
</div>
...
<script>
   ...
   function getNewProduct() {
      ...
      $.ajax({
         type: 'GET',
         url: "{{ route('get.new.product') }}",
         data: {
            page: newPage
         },
         success: function(response) {
            if (response.success) {
               if (response.newproducts.length > 0) {
                  var products = response.newproducts;
                  var productsHtml = '';
                  products.forEach(function(product) {
                     ...
                     var discountBadge = '';
                     if (product.discount_quantity > 0 && product.status == 'Active') {
                  ...
                  $('#newProductContainer').append(productsHtml);
                  newisLoading = false;
                  newPage++;
               } else {
                  $('#loadMoreNewProductButton').text('End').prop('disabled', true).addClass('sold-out-ticket');
               }
            }
         },
   ...
   $('#loadMoreNewProductButton').click(function() {
      getNewProduct();
   });
</script>
